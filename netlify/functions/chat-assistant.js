...
function normalizeStructuredResult(raw, today, existingCategories = []) {
  ...
  const isGenericCategory = GENERIC_CATEGORY_SET.has(categoryText)

  const knownCategory =
    categoryText && !isGenericCategory && (EXPENSE_CATEGORY_SET.has(categoryText) || INCOME_CATEGORY_SET.has(categoryText) || existingCategorySet.has(categoryText))
      ? categoryText
      : null
