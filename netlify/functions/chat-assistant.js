...
- 4요소를 모두 갖췄거나(또는 역질문으로 보충) 기록 의도가 유지되면 **add_ledger_entry** 로 즉시 등록하고, **적요(\`summary\` 키워드)** 기준으로 **"돼지국밥 점심 등 ₩… 식비로 기록했습니다"** 식 **짧은** 한 문장으로 보고해라(장문·서술·수식어로 원장을 설명하지 말 것).
...
- **\`summary\` (적요·필수):** **핵심 키워드만** 공백으로 연결. **서술어·~함·먹음·샀다** 금지. 시간은 **"점심" "저녁"** 처럼 **짧은 명사**로(예: \`돼지국밥 점심\`, \`미역국밥\`). "지기"·"AI" 금지.
- **\`detail_memo\` (선택):** 추가로 적을 말**만**; 없으면 **생략**. 적요를 **한 번 더 복붙하지 말 것.**
...
**[필수 4요소 확인]**
1) **적요(키워드)** — tool의 \`summary\`에 반영. 대화는 서술해도 되지만 **원장 적요**는 **키워드**만(위 규칙).
...
const TOOLS = [
  ...
  },
  {
    ...
    function: {
      ...
      description:
        '필수: summary(적요 키워드)·정확한 금액·날짜(YYYY-MM-DD)·category(Enum). summary는 서술어 없이 **명사/키워드**만(예: "돼지국밥 점심"). detail_memo는 선택. 내용/금액/날짜가 비면 도구 금지. category는 애매할 때만 역질문. 임의 분류명 금지.',
      parameters: {
        type: 'object',
        properties: {
          type: {
          ...
          summary: {
            type: 'string',
            description:
              '**적요(원장 제목)**. 핵심 키워드만, 공백으로 연결. 서술어·~함·먹음·샀다 금지. 예: "돼지국밥 점심", "미역국밥", "스타벅스 아아".',
          },
          detail_memo: {
            type: 'string',
            description: '**추가 메모**(선택). 없으면 생략. 적요를 반복하지 말 것.',
          },
          account: {
            type: 'string',
...
**add_ledger_entry (신규 등록):** [필수] **summary**(적요·키워드, 서술어 금지), 금액, 날짜, category(Enum). **detail_memo**는 선택. 금액/날짜/적요가 비면 **도구 호출 금지**. **택시·급여** 등은 category 재승인 없이 tool. 애매할 때만 category 역질문.
